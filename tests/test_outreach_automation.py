...
class AutomationTests(unittest.TestCase):
    ...
    def test_switches_are_off_until_turned_on(self):
        self.assertEqual(settings(self.conn, user_id=USER), {"auto_drafts": False, "bounce_recovery": False, "scheduled_sending": False, "follow_up_review": False})
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            with TestClient(app) as client:
                self.assertEqual(client.get("/api/v1/outreach/automation", headers=AUTH).json(),
                                 {"auto_drafts": False, "bounce_recovery": False, "scheduled_sending": False, "follow_up_review": False})
                saved = client.put("/api/v1/outreach/automation", headers=AUTH, json={"auto_drafts": True})
                self.assertEqual(saved.status_code, 200, saved.text)
                self.assertEqual(saved.json(), {"auto_drafts": True, "bounce_recovery": False, "scheduled_sending": False, "follow_up_review": False})
